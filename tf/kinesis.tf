...
resource "aws_kinesis_firehose_delivery_stream" "metric_data_firehose" {
  name        = "network-security-${var.cluster_name}-metric-data-firehose"
  destination = "extended_s3"

  extended_s3_configuration {
    role_arn            = aws_iam_role.kinesis_firehose_stream_role.arn
    bucket_arn          = aws_s3_bucket.metric_data_bucket.arn
    buffer_size         = 64
    buffer_interval     = 60
    s3_backup_mode      = "Enabled"
    prefix              = "metric/year=!{timestamp:YYYY}/month=!{timestamp:MM}/day=!{timestamp:dd}/"
    ...
    s3_backup_configuration {
      role_arn   = aws_iam_role.kinesis_firehose_stream_role.arn
      bucket_arn = aws_s3_bucket.metric_data_backup_bucket.arn
      prefix     = "backup"
    ...
    data_format_conversion_configuration {
      ...
      schema_configuration {
        ...
        table_name    = aws_glue_catalog_table.metric_table.name
        role_arn      = aws_iam_role.kinesis_firehose_stream_role.arn
        region        = var.region
      }
    }
  ...
  depends_on = [
    aws_glue_catalog_table.metric_table
  ]
}
